...
def setup(args, cfg, train_df, valid_df):
    ...
    if cfg['dataset']['name'] == 'FaceSeqDataset':
        if 'stack' in cfg['dataset']['params'] and cfg['dataset']['params']['stack']:
            train_images, train_labels, train_df = get_stacked_labels(train_df, cfg)
            valid_images, valid_labels, _ = get_stacked_labels(valid_df, cfg)
        else:
            train_images, train_labels = get_faceseq_labels(train_df, cfg)
            valid_images, valid_labels = get_faceseq_labels(valid_df, cfg)
    else:
    ...
    train_loader = builder.build_dataloader(cfg, data_info={'imgfiles': train_images, 'labels': train_labels}, train_df=train_df, train_images=train_images, mode='train')
    valid_loader = builder.build_dataloader(cfg, data_info={'imgfiles': valid_images, 'labels': valid_labels}, train_df=train_df, train_images=train_images, mode='valid')
...
def test(args, cfg, test_df):
    ...
    test_images = [osp.join(cfg['dataset']['data_dir'], '{}.png'.format(_)) for _ in test_df['image_id']]

    if cfg['dataset']['name'] == 'SingleBengaliDataset':
        test_labels = list(test_df[cfg['dataset']['target']])
    elif cfg['dataset']['name'] == 'BengaliDataset':
        test_labels = []
        for rownum in range(len(test_df)):
            rowlabel = {
                'grapheme_root': test_df['grapheme_root'].iloc[rownum],
                'vowel_diacritic': test_df['vowel_diacritic'].iloc[rownum],
                'consonant_diacritic': test_df['consonant_diacritic'].iloc[rownum]            
            }
            if re.search(r'Quat|Grapheme', cfg['model']['name']):
                rowlabel['grapheme'] = test_df['grapheme'].iloc[rownum]
            test_labels.append(rowlabel)
    ...
    if 'grapheme_model_checkpoint' in cfg['model']['params'].keys():
        cfg['model']['params']['grapheme_model_checkpoint'] = None
    model = builder.build_model(cfg['model']['name'], cfg['model']['params'])
