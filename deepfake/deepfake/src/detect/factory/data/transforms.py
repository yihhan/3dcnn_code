...
class FetchImage(object):
    ...
    def __call__(self, results):
        results['filename'] = '.'
        results['img'] = results['img_info'].cpu().numpy().squeeze(0)
...
def load_pipeline(resize_to, mean, sdev, image_range, input_range, size_divisor=32):
    img_norm_cfg = {
        'mean':  mean,
        'sdev':  sdev,
        'image_range': image_range,        
