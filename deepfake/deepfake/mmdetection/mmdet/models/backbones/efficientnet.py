...
class EfficientNet(nn.Module):
    ...
    def __init__(self, phi=0, norm_eval=False, override_params=None):
        super().__init__()
        self.model_name = 'efficientnet-b{}'.format(phi)
        self.norm_eval = norm_eval
        self._check_model_name_is_valid(self.model_name)
        blocks_args, global_params = get_model_params(
            self.model_name, override_params)
    ...
    def init_weights(self, pretrained=None):
        if pretrained is not None:
            load_pretrained_weights(self, self.model_name, load_fc=True)
